...
public class TextFileRead {
    public static void main(String[] args) {
        ...
        try {
            ...
            fis = new FileInputStream("src/실습1_풀이/score.txt");
        } catch (FileNotFoundException e) {
            System.out.println(e);
        }
        // 스캐너 입력은 콘솔 이외에 파일로부터도 읽어 들일 수 있음
        Scanner sc = new Scanner(fis);
        while(sc.hasNextLine()) { // 읽어들일 다음 라인이 있는 지 검사
            String line = sc.nextLine(); // 개행문자(엔터키) 기준으로 읽어들임
            String[] strArr = line.split(" ");
            treeSet.add(new StudentInfo(strArr[0],
                    Integer.parseInt(strArr[1]), // 문자열을 정수로 변환해줌
