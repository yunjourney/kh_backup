...
public class QueueMainEx2 {
    ...
    public static void main(String[] args) {
        ...
        while(true) { // "q/Q" 문자가 들어 올 때까지 계속 반복 수행
            System.out.print(">>"); // 터미널에서 입력 화면처럼 보여 주기 위해서 만듦
            Scanner sc = new Scanner(System.in);
            String in = sc.nextLine().trim(); // 입력 받은 문자열에서의 앞뒤의 공백을 지워줌
            if(in.equalsIgnoreCase("q")) {
                System.exit(0); // 강제 종료 시킴, System Call
                // equalsIgnoreCase() 문자열 비교 시 대소문자 구분 안 함
                ...
                int cnt = 0;
                for(String e : queue) {
                    cnt++; // 명령 history 앞에 숫자를 붙여주기 위해 만든 변수
